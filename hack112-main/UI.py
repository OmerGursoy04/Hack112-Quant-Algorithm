    ...
    for i in range(3):
        ...
        if mouseX > x0 and mouseX < x1 and mouseY > y0 and mouseY < y1:
            ...
            if i == 0:
                ...
                print(app.userTickerInput)

                run_quant(K=strike, σ=iv, fulldate=app.dates[app.dateSubmission + app.dateIndex - 10], ticker=app.userTickerInput) #Call the graph function. 
...
def drawInfoScreen(app):
    info = 'This project was built by a group of freshman 15-112 students from Stever House during the annual Hack-112 hackathon. It uses a combination of web scraping and the Black-Scholes algorithim to predict \
    information about various stock option calls.'
    drawLabel('About', app.width/2, app.height/6 + 20, size = 20)
    drawLabel('This project was built by a group of freshman 15-112 students from Stever House during the annual Hack-112 hackathon.', app.width/2, app.height/6 + 40, size = 16) 
